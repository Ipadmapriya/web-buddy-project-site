...
const EducationForm = ({ onFormSubmit, initialData = [], userType }: EducationFormProps) => {
  ...
  // Helper function to get gradient class based on education level
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          {educations.map((education) => (
            <div 
              key={education.id} 
              className={`p-4 border rounded-lg space-y-4 shadow-md transition-all duration-300 hover:shadow-lg ${getGradientClass(education.level)}`}
            >
